...
- `transcript.go` — the single owner of the scrolling rendered transcript region (layout, scroll, follow, render). Committed-history rendering is incremental: a per-settled-message memo (`units`) stores each committed message's rendered bytes and row accounting, so committing a new turn renders only that turn and concatenates the prior units, and a new turn's Begin serves the busy prefix from that memo instead of re-rendering it. The settled-message emitter (`committedEmission`) is shared by the memo and the full-history loop, so incremental output is byte-identical to a fresh render. It splits invalidation into two tiers: `layout.dirty` marks a render-visible change (stream deltas and tool observations included), while `busyPrefixDirty` marks a genuine committed-history change (turn commit, user append, expansion toggle, resize, theme change, or a tool observation landing on a committed turn). An in-place committed change drops the memo scoped to the inputs that feed it: an expansion toggle, a committed tool observation, or a focus-marker move marks only the unit(s) whose flow draws the affected block (`invalidateCommittedUnit`), while a width, theme, or expand/collapse-all change re-wraps everything and drops the whole memo (`invalidateCommittedMemo`). Pure appends and live-tail-only activity leave it intact so a commit extends rather than rebuilds. Live-only activity — per-delta stream snapshots, live tool observations on the running turn, and idle/timer/clock frames — dirties only the former, so a long, tool-heavy live turn never forces a full committed-history rebuild.
...
- `markdown.go`, `flowrender.go`, `expansion.go`, `collapsefocus.go`, `scrollreg.go`, `selection*.go`, `clipboard.go`, and the `internal/osc52` package — rendering: glamour markdown, expand/collapse of tool and chain-of-thought blocks, drag-select text, OSC 52 clipboard escape sequences. Committed output is full glamour; a *streaming* block renders cheaply and converges to that same glamour output at commit (so committed bytes never depend on the live path). A live chain-of-thought body is plain, style-free text de-emphasized (faint+italic, no hue) by its own pane — no markdown, no SGR — while a live answer keeps a simplified inline-emphasis wrap. The reasoning surface's only color is the theme's curated muted `thinking` hue, carried on the pane border and the 🤔 marker rather than the glyphs, so chain-of-thought never competes with the tool-category hues.
