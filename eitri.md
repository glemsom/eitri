...
- Key settings: `model` (default `deepseek-v4-flash`), `reasoning_effort` (default `low`; tiers `low`/`medium`/`high`/`max`; raised to trade cost for deeper chain-of-thought, lowered to save cost), `thinking_enabled` (default `true`; when off, runs emit non-thinking requests with no thinking toggle and no `reasoning_effort`, exactly like the compaction summarizer), `max_turns` (cap on tool-loop iterations), `compaction_fraction` (context-utilization trigger for auto-compaction), `theme` (Markdown render theme, which also selects the TUI chrome palette; default `dark`; supported `dark`/`light`/`dracula`/`tokyo-night`/`pink`/`notty`/`auto`; `dracula` re-skins the chrome with a purple-accented palette, all other values keep the default chrome palette; unknown values fall back to `dark` for Markdown and the default chrome palette, printing a one-time `unknown theme …` warning in the TUI status band on startup), `extra_writable_paths` (additional writable roots for write/edit).
