...
- **Composer.** In the TUI, plain `Enter` submits the prompt; `Shift+Enter` inserts a newline for multi-line drafts. The composer grows with the draft within the fixed bottom band (one row per line, up to a bound), then scrolls internally so a long draft never spills into the transcript; the status strip and slash-completion list stay pinned above it (issue #121). The composer's caret is the terminal's hardware cursor, not a software-rendered cell: it tracks the true edit position through typing, soft wraps, growth, and internal scroll (issue #168). Its style is an explicit policy (issue #170): a steady (non-blinking) block, requested deliberately rather than inherited from the terminal's own cursor settings — a terminal that ignores the shape or steady-bit requests falls back to its own default caret, never hidden by the style policy. The caret only shows while the composer is the active editing surface: it hides on the Settings surface, the max-turns continuation prompt, the review panel, and while an agent turn is running, and returns as soon as the composer is editable again (issue #169).

- **Transcript copy.** In the TUI, `ctrl+o` copies the full conversation transcript (prompts, answers, reasoning, tool entries) to the system clipboard; the `/copy` slash command does the same from the command surface. A click-drag over the transcript highlights a cell range (reverse video) and releasing the drag copies just that snippet — wrapped lines and ANSI-styled rows included — so a single code block or error can be extracted without the whole session (issue #124). Copy reports success/failure as a status note and never mutates the transcript, the composer, or the agent loop (issues #123, #124). When the system-clipboard path fails — e.g. no `xclip`/`wl-clipboard` installed — the copy falls back to the OSC 52 terminal-clipboard sequence (issue #201): the terminal itself (Ghostty by default, others with OSC 52 support) puts the text on the system clipboard, no external tools required. The fallback only fires when stdout is a TTY, so piped/non-terminal output never receives escape garbage.
...
- **Styling identity.** The TUI surface uses a restrained dark palette with a single agent accent: user prompts render as plain markdown, assistant answers as left-bordered panes, and errors get an error-colored border. Tool entries (⊕) render colorized by category — shell, file, web, and skill each draw a distinct hue from the theme palette (issue #181) — and the thinking hint (🤔) renders italic in the accent hue so it reads as a distinct treatment from answer bodies. Tool outcome (✓/✗) and error (⚠) markers stay as they were; every colored element keeps its glyph, so meaning never depends on color alone. The right context rail (`ctrl+b`) colorizes each of its sections — STATS, CONTEXT, MODEL — with a per-section hue from the same palette; STATS renders its live readouts — cache hit %, cost, turns, token in/out — as numeric lines only; the rail stays read-only against the agent loop. The fixed bottom band (status strip + slash completion + composer) is framed by an accent separator row, and the status strip renders in the accent hue to match the colorized rail. The chrome's colors and derived styles live in a `Theme` palette registry (`internal/tui/styles.go`, issue #178): the default theme is the pre-seam dark palette, every chrome consumer renders through the model's theme field, and a new palette is a registry addition with no consumer change. The chrome follows the `theme` selection (issues #179, #180): `dark` (the default) keeps the pre-seam palette, and `dracula`, `tokyo-night`, `pink`, and `light` each select a curated palette with hues matching their glamour Markdown counterpart; `auto` resolves like the renderer — light palette on a light terminal, dark on a dark one — and `notty` (unreachable in the TUI) keeps the default palette deliberately. Only unknown values fall back to the default palette, matching the renderer's fallback, so chrome and Markdown never disagree. All palette colors are hex; on the Charm v2 stack (issue #145–#149) lipgloss/bubbletea
