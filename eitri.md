...
- **Composer.** In the TUI, plain `Enter` submits the prompt; `Shift+Enter` inserts a newline for multi-line drafts. The composer grows with the draft within the fixed bottom band (one row per line, up to a bound), then scrolls internally so a long draft never spills into the transcript; the status strip and slash-completion list stay pinned above it (issue #121). The composer's caret is the terminal's hardware cursor, not a software-rendered cell: it tracks the true edit position through typing, soft wraps, growth, and internal scroll, and blinks/shapes per the terminal's own cursor settings (issue #168). The caret only shows while the composer is the active editing surface: it hides on the Settings surface, the max-turns continuation prompt, the review panel, and while an agent turn is running, and returns as soon as the composer is editable again (issue #169).
