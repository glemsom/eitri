...
5. **Merge queue:** After all workers finish, the dispatcher merges PRs one at a time (`gh pr merge --squash`), rebasing each PR branch onto the latest `origin/main` first. `--delete-branch` is intentionally not used: gh cannot delete the head branch while it is checked out in its worktree (it fails with exit 1 after the merge already lands), so the merge is judged by PR state instead and the remote branch is deleted explicitly. A PR is merged **only** when bash tracks the latest test verdict = `PASS` **and** the latest review verdict = `APPROVED` (the dual-gate precondition; `can_merge_issue`). PRs that failed the gates are left open with a "needs human" comment and count as leftovers. If a rebase conflicts, the dispatcher spawns a focused `eitri -b` resolution run inside that worktree, capped at 3 attempts per PR; past the cap the PR is left open with a comment and the dispatcher moves on. Merging is serialized because two concurrent merges would race (the second PR goes stale and GitHub refuses to merge).
