...
- The render primitives (`render.go`, `live_markdown_cache.go`) depend on package-global state read all over the package: `Theme` (~30 fields), the i18n helper `g()`, `motionEnabled()`, `busySpinnerFrames`, `RenderMarkdown`, `Phase`, `phaseVerb`/`forgeVerb`. Extracting them means exporting or moving that shared surface too.
...
3. Flip package-`tui` call sites to the imported seam; the package-wide unexported globals the leaf no longer sees (`Theme`, `g()`, spinner state) either move with it or become explicit parameters or fields on the seam.
