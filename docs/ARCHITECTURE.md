...
The former LLM-history store. After issue #1241 the loop's session-backed history adapter reads and writes the canonical conversation store (`internal/session`) directly, so UI and batch runs no longer use this store — it survives only as the parity baseline the canonical store was driven against (`internal/session/exchange_parity_test.go`, `internal/runner/loop/adapters_test.go`), and is scheduled for deletion (issue #1242). `DefaultMaxExchanges` aliases the single canonical default in `internal/message` (issue #1239).
...
The sync seam is the documented, unit-tested boundary where the loop's history shape and the snapshot facade meet (issue #1235): it converts a run's LLM history (`[]EitriMessage`, system prompt prepended on reads) into the flat conversation shape (`[]Message`, system prompt stored separately) and strips the leading system message. `SyncHistoryToConversation` is the single home of the strip-system-message invariant (ADR-0028). After issue #1241 the session-backed path no longer funnels through here — the loop reads and writes the canonical conversation store directly — so the seam remains only where a facade is derived from `HistoryManager.History()`: the batch/sub-agent `runCompleter.buildUISession` snapshot facade. The UI live-sync and manual compaction (`CompactSession`) operate on the canonical store through the same adapter.
