...
Compaction is gated by estimated-token thresholds (`compactor.Thresholds`): it triggers when total estimated tokens exceed `HighWater` and stops once below `LowWater`; an individual message must exceed `MessageSizeThreshold` (default 2000 estimated tokens) to be eligible. The runner derives these from `compaction_threshold_percent` (high-water, default 90) and `compaction_low_water_percent` (default 30) of the context window, gated by `compaction_enabled`. Auto-compaction runs after each complete agent turn for both UI and batch parent runs through the shared `autoCompactAfterTurn` step (`OnTurnComplete` / the batch turn completer), so the config settings are honored identically in both modes — a batch run that exceeds the high-water mark compacts to below the low-water mark just like a UI run. Sub-agent runs persist snapshots only and do not auto-compact. On-demand compaction runs via `RunService.CompactSession`, exposed as `POST /api/sessions/{id}/compact` and the `/compact` slash command.
...
| `compact.go` | `autoCompactAfterTurn()` — the shared auto-compaction step for UI and batch parent runs (issue #1093): builds the summarization LLM service, gates on high-water, runs the compactor with the configured thresholds/salience/tool-call retention, and restores the compacted history into the history manager |
