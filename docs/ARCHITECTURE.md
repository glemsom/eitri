...
4. **UI session manager** (`session.Manager`) — in-memory browser-facing session state with `browser_id` ownership and max-session cap (default 10); the single conversation store for UI and batch runs (issue #1241)
5. **Skills service** (`skills.Service`) — scans Agent Skills roots, resolves precedence, exposes effective/shadowed/invalid records
6. **Run service** (`runner.RunService`) — run lifecycle, agent loop, SSE broadcast via `runstate.State`
7. **Report service** (`report.Service`) — constructed once from the persister and injected via `ServerConfig.ReportService`; the report handlers consume it instead of building a per-request service (issue #1206)
8. **HTTP server** (`api.NewServer`) — registers routes via `net/http` (Go 1.22+ ServeMux); uses `session.Manager`, `runner.RunService`, `report.Service`, `config.Manager`, and `skills.Service`
...
| `exchange.go` | `DefaultMaxExchanges`, `TrimExchanges`, `RepairPendingToolUse` — the exchange-cap sliding window and pending-tool-use repair as pure functions over the flat `Message` shape (issue #1239): the two history behaviours the canonical session store enforces |
| `exchange_test.go` | Unit tests for the exchange-cap sliding window and pending-tool-use repair |

`EitriMessage` is the canonical message type at the loop's LLM boundary: the
`loop.HistoryManager` adapters read and write it (the loop converts it to
litellm for the request). `Message` is the flat serialization/conversation
shape used by the canonical session store and persisted snapshots; the
conversion between the two happens at the store/LLM boundary, and the
former `internal/message/sync.go` module that bridged the loop's history shape
and the snapshot facade (issue #1235) was deleted with the old LLM-history
store (umbrella #1231, issue #1242).

`exchange.go` holds the exchange-cap sliding window and pending-tool-use repair
(issue #1239) as pure functions over the flat `Message` shape:
`TrimExchanges` removes the oldest exchanges (a user message plus everything
after it until the next user message, keeping the trailing assistant/tool tail)
when the user-message count exceeds the cap, and `RepairPendingToolUse` closes
a trailing unresolved assistant tool call with a synthetic tool error result so
a resume never appends a user message after an unclosed tool use.
`DefaultMaxExchanges` (150) is the single canonical default; the session store
resolves to it.
...
`GenerateRunID` derives a run's short hex identifier from (session, started-at). It is called exactly once per run — at run start by each transport (UI, batch, sub-agent) — and the resulting ID is plumbed through the run options (`loop.RunOpts.RunID`, stamped onto every HTTP trace) and the terminal seam (`runCompleter.runID`, stamped onto the persisted timeline), so the timeline, SSE-derived events, and HTTP traces of one run share a single identifier by construction and turns stay correlatable to their traces by ID (issue #1234; correlation issue #988). No call site recomputes the ID.
...
The canonical store is the **single conversation store** for UI and batch runs (issue #1241): the loop's session-backed history adapter (`loop.sessionHistoryManager`) reads and writes it directly, so the UI conversation, the LLM request history, and the persisted snapshots all read the same messages — the former per-turn history→conversation copy in the turn-completer is gone, and the old LLM-history store (`internal/history`) plus its sync layer were deleted once no references remained (umbrella #1231, issue #1242). It enforces the history behaviours the old store had (issue #1239): a per-session **exchange-cap sliding window** (default `message.DefaultMaxExchanges` = 150; configured via the `WithMaxExchanges` constructor option — production wires `cfg.MaxHistory` in `cmd/eitri/main.go`) trims the conversation on every append path (`AppendMessage`, `AppendToConversation`) with the exact sliding-window semantics of the former store's trim, and `RepairPendingToolUse` closes a trailing unresolved assistant tool call with a synthetic tool error result before a resume. `ReplaceConversationMessages` does **not** trim — matching the former `RestoreHistory` (compaction / history write-back is never trimmed). On read, the adapter's `History()` filters **UI-only empty assistant placeholders** — the bare `{role:"assistant"}` messages that `AppendComponent`/`SetQuickReplies` append when the last conversation message is not an assistant message (the second and subsequent component-emitting tool calls of a turn) — because the old LLM-history store never carried them and they serialise as content-less assistant messages that some providers reject; the canonical store keeps them as UI component targets (compaction reads and rewrites the conversation through this LLM-view `History()`, so a compaction also drops the empty placeholder scaffolding — components attached to real assistant messages survive). Reads of the canonical conversation from paths that can run concurrently with a live run — the session-backed adapter's `History()` (manual compaction has no active-run guard), the UI snapshot source, and batch run-start's `extractLastMessages` — go through the locked `CopyConversation` snapshot, never the shared reference returned by `GetConversationShared` (whose lock is released before the caller reads; issue #1241 fix round). Persisted conversations are restored on demand via `POST /api/sessions/{id}/load` (`LoadSessionFromDisk` → `Manager.LoadFromDisk`), which restores the conversation into this store — the former separate startup `RestoreHistory` hydration is gone.
...
| `tracequery.go` | `QueryTraces` — `TraceFilter`/`TracePage` query surface over the persisted trace archive; `AggregateTraces` delegates the window fold to `internal/debug` (single aggregation owner, issue #1240) |
...
Enforces a 1 GiB retention cap by default: `Prune` evicts the oldest timeline and trace files across all sessions when total size exceeds the cap. Trace persistence is asynchronous (`SaveTraceAsync`) through a bounded worker queue (256) so disk I/O never blocks the HTTP path; shutdown `Flush` drains the queue and re-scans the debug recorder so nothing is lost. Traces of permanently deleted sessions (no `session.json` on disk) are never written. The "is the session still on disk?" check has a single owner — `sessionExistsOnDisk` (issue #1237) — used by the snapshot loader, `SaveTrace`, `Flush`, and the trace query surface, so "permanently deleted" means the same thing on every trace save / flush / query path; the check is `IsNotExist`-aware, so a stat failure other than "no `session.json`" is surfaced at each site rather than treated as deletion. `QueryTraces` filters by session/provider/model/time with limit/offset pagination. Window aggregation (error rate, p50/p95 latency, token totals) for the debug UI has a single owner — `debug.AggregateTraces` (issue #1240) — and `AggregateTraces` delegates to it instead of re-implementing the fold. See ADR-0016.

Trace identity is single-owner across restarts (issue #1236): `Restore` marks every restored trace as already persisted (so the shutdown `Flush` never re-writes it), and `SaveTrace` refuses to overwrite an existing trace file, returning `ErrTraceExists` instead of silently clobbering an archived trace with a colliding fresh ID. The recorder's ID generator is reseeded past the restored archive on startup (see `internal/debug/`), so fresh `trace_N` IDs always advance past the archive and collisions cannot occur in the first place.
...
| `recorder.go` | `Recorder` — ring buffer of HTTP request/response traces for diagnostics. Each `HTTPTrace` is enriched with provider-reported usage, `finish_reason`, model name, retry attempt number, time-to-first-byte, time-to-first-token, and the run/turn correlation IDs. `LoadAll` (startup restore) reseeds the ID generator past the largest restored `trace_N`, so fresh IDs always advance past the archive and never collide with a restored trace file (issue #1236). |
| `aggregate.go` | `TraceAggregate` / `AggregateTraces` — the single window-aggregation implementation (count, error rate, p50/p95 latency, token totals, window bounds) shared by the persisted archive aggregate endpoint; `persist.AggregateTraces` delegates to it (issue #1240) |
...
| `run.go` | `StartRun()` — validates config, snapshot runtime limits, resolves skill context, calls the shared `prepareRun` seam, starts agent loop; wires a UI-mode `runCompleter` as the per-turn `TurnCompleter` (ADR-0028) with a plain-`CopySession` terminal snapshot source; every exit path finishes through the single run-end seam `runExit` (run_exit.go), with the UI transport's per-reason work (`uiExitWork`: run-end append, SSE closing events, crash dump, session-status broadcast) dispatched by the shared exit switch (ADR-0029) |
...
| `run_completer.go` | `runCompleter` — the unified per-turn run-completer for UI, batch, and sub-agent runs (issues #1107, #1201; ADR-0025, ADR-0028): persists a running-status snapshot after each complete turn, runs the shared `autoCompactAfterTurn` step and re-snapshots when compacted, and on every exit path writes a terminal snapshot + timeline through the run-end seam (`runExit` → `terminal`). The conversation source is parameterized through the `loop.HistoryManager` seam: for UI/batch runs the session-backed adapter reads and writes the canonical conversation store directly, so the UI snapshot source is a plain `CopySession` — the per-turn history→conversation copy is gone from this path (issue #1241); sub-agent runs stay request-based. Batch/sub-agent runs build the snapshot facade from history via `buildUISession`, which converts the loop's LLM-boundary `[]EitriMessage` to the flat `[]Message` inline (the former `internal/message` sync module was deleted with the old history store — issue #1242). The terminal seam persists the run's timeline under the run ID plumbed in at construction (`runID`, generated once at run start — issue #1234), never an ID recomputed from (id, started-at); UI runs set `terminalSnapshotSource` to a plain `CopySession` so the terminal snapshot preserves UI-only messages exactly (issues #1238, #1241) |
| `run_exit.go` | the single run-end terminal seam for UI, batch, and sub-agent runs (issue #1238; ADR-0028/0029): `classifyRunExit` (the shared exit taxonomy — idle on completion/cancellation/max-turns, error on true failure), `exitWork` (the single per-reason exit switch — adding a termination reason touches exactly this file), and `runCompleter.runExit` (classify → dispatch per-reason transport work → terminal snapshot + timeline → optional `afterTerminal` work). Directly callable, so exit paths are unit-testable without a full run (`run_exit_test.go`) |
...
| `batch.go` | `BatchRun()` — headless batch execution with token streaming to `io.Writer`; streams both ordinary text tokens and reasoning/thinking deltas to stdout, the latter delimited by `[thinking]`…`[/thinking]` markers so reasoning is distinguishable from final text (issue #1095); shares the unified `prepareRun` seam with UI runs (skills catalog, `skill` tool, `max_output_tokens`, prompt-cache key, thinking level, `EndSession` cleanup, loop-panic crash dumps — ADR-0024); persists session snapshots per turn (via the `TurnCompleter` seam) and finishes every exit path through the single run-end seam `runExit` (run_exit.go: terminal snapshot + timeline, status classified by the shared exit taxonomy — ADR-0029: idle on cancellation/max-turns, error on true failure; the CLI exit code stays driven by the returned error; batch runs have no per-reason exit work and pass a nil `exitWork`); auto-compacts the conversation history via the shared `autoCompactAfterTurn` step when the configured high-water mark is exceeded (issue #1093); sub-agents spawned from batch mode auto-compact too (issue #1096); session ID auto-generated via the shared `runJobID` helper (ADR-0025), title from `session.TitlePreview` |
| `loop/` | Agent turn loop (`loop.go`, `loop_helpers.go`, `tool_call.go`, `debug.go`) + `adapters.go` (confirmation seam: `ConfirmationFunc`, `NewFuncConfirmer`; history seam: `HistoryManager` with `sessionHistoryManager` and `requestHistoryManager` adapters, both supporting `ReplaceHistory` so auto-compaction writes the compacted history back). `sessionHistoryManager` (UI/batch) wraps the canonical conversation store (`internal/session`, issue #1241); `requestHistoryManager` (sub-agents) wraps `*litellm.Request`. Streaming (ChatStream consumption) lives in `loop.go` |
...
- `eitri-settings`: settings-page interactivity (dirty guards, model refresh, test connection, provider-default endpoint reset, default-prompt preview + reset-to-default).
