...
The read side returns typed artifacts instead of raw bytes: `LoadTimeline` → `*timeline.Timeline`, `LoadSession` → `*session.UISession` (nil when no snapshot exists), `LoadTrace` → `*debug.HTTPTrace`, and `ListTraces` → `[]*debug.HTTPTrace` (unreadable/corrupt files skipped), so consumers — the Session Report, the runner's on-demand session restore, and the cleanup UI — never re-unmarshal persisted artifacts. `LoadSessionInfo` and `QueryTraces` stay the lightweight metadata/query surfaces.
