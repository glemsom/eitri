...
- `eitri-stream`: opens `/api/sessions/{id}/stream` only after chat POST trigger; parses JSON envelopes; batches display-only tokens; handles run phases, no-dead-air, reconnect state, cancellation UI, render endpoint dispatch, and final Markdown render by `message_id`. The stream lifecycle is hardened: a run finalizes exactly once per run (duplicate/replayed `done` packets are ignored via a run-status guard), tool-card elapsed timers are stopped when their cards leave the DOM (not only on `tool_result`/FIFO eviction), and a transient EventSource `onerror` during an active run preserves tool activity/elapsed data so cards resume after reconnect with replay-stable keys instead of duplicating. (issue #1070)
