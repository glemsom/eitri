...
| `subagent.go` | `SpawnSubAgent()`, `CollectSubAgents()` — sub-agent lifecycle management, `buildBaseToolRegistry()` (base tools + `skill`; no `delegate`/`collect`/`render_quick_replies`); persists each sub-agent run as a child session on disk (snapshot + traces + timeline under `~/.eitri/sessions/<taskID>/`) in both UI and batch modes; auto-compacts the sub-agent's request-based history via the shared `autoCompactAfterTurn` step with the parent's compaction settings (issue #1096) |
