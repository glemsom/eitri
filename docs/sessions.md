...
| `messages.jsonl` | **Message-layer transcript**: one JSON line per provider request/response turn — the exact messages sent to and received from the model. |
| `trace-request.http`, `trace-response.http` | Raw HTTP bodies; written only in debug mode (`-d`). |

## `messages.jsonl` schema

Each turn appends two lines: one request record, one response record.

Request record:

```json
{"ts":"…","dir":"req","model":"<model>","messages":[<provider.Message wire shape>],"tools":["bash", …]}
```

- `messages` is exactly what went over the wire this turn: system prompt,
...
```
eitri session list                  # GUID, last activity, turn count, model — newest first
eitri session show <guid>           # one compact line per turn: model, tools, calls, tokens, errors, answer preview
eitri session show <guid> --turn N  # full pretty-printed JSON of that turn's req+resp records only
...
eitri session grep <pattern> [guid|all] [-full]
                                      # turns whose content/reasoning/tool args match, with snippets;
                                      # -full prints the complete matching field text instead of a snippet
eitri session talk <guid>             # the full message stream as plain text: [N] role: blocks, untruncated
...
`talk` is the read-the-messages command: it renders actual message bodies,
where `show` renders per-turn metadata. Tool calls render as
...
turn (spike in tokens, an error, an odd tool call) → `--turn N` for full
