...
- **Reading:** collapsible stream per-turn (auto-collapse after turn). **Compaction:** `[compacted]` marker + cache hit-ratio gauge. **Skills:** show detected and currently-activated skills; slash-command activation (`/skillname`) and `/` completion in the composer. (Tickets #17/#18/#21, eitri.md §2.3)
