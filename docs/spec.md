...
- **Surfacing:** reasoning arrives on streamed `reasoning_content`, hoisted per-turn into the assistant message — never merged into answer `content`. TUI: a per-turn collapsible thinking block streams live from the reasoning seam, collapses back to a one-line hint (`🤔 <token estimate> · <effort>`) once that turn's final answer lands, and expands in place (no alt-screen) on demand. Batch stdout: suppressed by default; `-v` enables. (Tickets #17/#85)
...
- **Anchored LLM summary:** evicted body → `## Objective` / `## Next Move` summary, capped 4,096 tokens, re-injected below the immutable base system prompt — the head shape is `[base system prompt, summary, verbatim tail]`, the base prompt preserved at `[0]` (spec §34 / #102, re-anchored per #103) — so the prefix stays a valid byte-stable cache head (#12). Generated by the same `deepseek-v4-flash` as a separate non-tool call (sub-cent); **fail-safe skip** if the summary prompt alone won't fit. (ADRs #21/#12 / Ticket #21)
...
- **Engine observer seam (event stream):** the TUI subscribes to the run engine's live typed event stream (`engine.SetListener`/`engine.Listener`) so it renders the run *as it streams* — thinking deltas, answer tokens, tool calls/results, per-turn usage, turn boundaries, and the compaction marker — instead of only re-rendering a finished transcript. The events carry the reasoning (`StreamEvent{Kind: ReasoningStream}`), tool-compression metadata (`ToolResultEvent`, spec §5), and compaction marker (`CompactedEvent`, spec §7) the panes need, so the TUI never re-parses the raw stream or internal message history. Unsubscribed runs (batch/headless) emit nothing and are byte-identical. (Issue #81)
...
- **Read-only project state:** the TUI surfaces the workspace directory the run operates in as an informational header above the transcript (workspace path), distinct from the user-editable composer, so the operator always knows which directory the agent acts on. (Issue #82)
- **Reading:** a per-turn collapsible thinking block streams live from the reasoning seam; collapsed it is a one-line hint (`🤔 <token estimate> · <effort>`), `tab` expands just that turn's block in place, and it auto-collapses back to the hint once the turn's final answer lands so the styled answer takes focus. **Compaction:** `[compacted]` marker + cache hit-ratio gauge. **Skills:** show detected and currently-activated skills. **Composer slash surface:** `/skillname` activates a skill, `/settings` routes to the settings modal, and a `/`-prefixed line shows a completion list mixing the built-in `/settings` command with matching skills; any other `/...` line remains a normal prompt, so slash handling never swallows input. (Tickets #17/#18/#21/#85; Issue #87; eitri.md §2.3)
- **Tool-call entries:** each tool call renders as a compact one-line `⊕ tool  args` entry in the transcript instead of a raw output dump. The result collapses to a line-count summary with the explicit `+N more` tail marker when compressed (spec §5), and file-edit tools carry a `[+N, −M]` line-delta tag; `alt+y` expands every entry to its full inline result, so nothing is ever silently truncated (there is always an expand path). The engine reports the compression metadata and file line delta as pure UI telemetry on `ToolResultEvent` (via `AgentOptions.ToolDelta` / the app's `fileLineDelta` seam), never affecting the run or its message history. (Issue #84)
- **Right context rail (Layout A):** `ctrl+b` toggles a fixed-width state pane beside the transcript — STATS (cache hit %, running cost, turns, token in/out), CONTEXT (active skills, session id, session temp path), and MODEL (provider/model/effort) — fed live from the telemetry and skill surfaces so the transcript itself stays clean. It auto-shows on wide, tall terminals (≥ 120 cols and ≥ 24 rows) and auto-hides narrower or shorter to preserve the primary buffer's full-width native selection and to leave the rail vertical room beside the composer band; `ctrl+b` toggles it on any size. The rail height-caps to the same visible height as the history region so the two panes form one coherent row. One pane for time (transcript), one for state (rail). (Issues #88, #109)
- **Review panel (Layout B):** `ctrl+d` opens a read-only changed-file review over the transcript — the "what did the agent actually change?" view (stories #3/#11/#46). It lists every file the agent touched with `[+N, −M]` delta and status (modified/added/deleted, derived from before/after content), lets the user inspect a focused file's inline diff without leaving the terminal (a **pure-Go diff engine**, no Node/no external render, honoring the single-binary/no-deps constraint of spec §12), and wires `open_in_browser` (the host-side tool, ADR-0001) as the escape hatch to the full file in the host browser/editor. The engine reports the before/after content + resolved host path on `ToolResultEvent` via the same `ToolDelta` seam; the panel never mutates the repo or the live agent loop. (Issue #90)
...
- **Event stream is tested at the engine seam.** The observer surface (issue #81) is asserted end-to-end against the fake/scripted provider — event *order* and *payloads* for reasoning/answer deltas, tool-call/results with compression metadata, per-turn usage, turn transitions, and the compaction marker — with no per-widget test seams. It also verifies batch behaves byte-identically: an unsubscribed engine emits no events (docs/spec.md §10).
- **Modules to test:**
