...
- **Live chain-of-thought is a plain, style-free body** (`renderLiveThoughtBody`): the raw text with no markdown parse and no SGR at all. The reasoning pane supplies italic styling (no hue — the muted reasoning hue lives on the pane border and the header mark, `≡` collapsed / `🧠` expanded); a body carrying its own SGR would reset the pane style mid-line — the failure the earlier "cheap emphasis" body kept hitting. While streaming, the pane border is dimmed to distinguish the state; the body itself stays identical to the committed pane. Both paths word-wrap at the content width, falling back to a hard break only for an over-long unbroken token, and trim leading and trailing newlines so live and committed bodies start identically. The tradeoff is that markdown syntax shows verbatim until commit.
- **The live answer keeps the cheap ANSI emphasis** (`renderCheapLiveBody`): simplified bold/italic/code/link handling plus one word-aware ANSI wrap, because the answer must still read as the answer while streaming. Like the reasoning path, it falls back to a hard break only for a token longer than the width, and trims leading and trailing newlines to match the committed glamour path.
