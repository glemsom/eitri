...
Provider behavior becomes data-driven by profiles: default base URL, API-key requirement, model-discovery path/parser, chat-completions path, extra headers, and provider-auth helpers. GitHub device flow remains Settings-triggered UX, but start/poll/auth-state encoding/auth resolution live with provider logic instead of Settings-only handler branches. `/responses`, Anthropic `/v1/messages`, and WebSocket transports stay deferred until existing streaming chat-completions contract proves insufficient.
