...
6. **Tool definitions stay in `internal/agent/`** with explicit Go structs + `JSONSchema() litellm.Schema` methods. A generic `SchemaOf[T]` helper (maps struct fields + `jsonschema:` tags to JSON Schema) reduces boilerplate.
