...
// applyDelta grows the in-progress assistant answer with one streamed delta by
// delivering an answerDeltaMsg through the model's Update seam.
func applyDelta(t *testing.T, m Model, delta string) Model {
	t.Helper()
	nm, _ := m.Update(answerDeltaMsg{delta: delta})
	return asModel(t, nm)
}
