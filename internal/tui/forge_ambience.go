...
// gradientKey packs the theme's blended palette into a comparable cache key.
func (th Theme) gradientKey() gradientKey {
	return gradientKey{colorKey(th.accent), colorKey(th.skill), colorKey(th.web)}
}
...
// gradientBandColors returns the fixed band colors for the theme's accent ->
// skill -> web sweep — the idle banner rules and the band panel frame.
...
// one per cell. Results are memoized because the band panel border is rebuilt
...
func (th Theme) gradientCorner(open, close string, col, width int) string {
	glyph := open
	if col != 0 {
		glyph = close
	}
