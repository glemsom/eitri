...
func TestRailRegressionTruncationAtMinimumAndWideWidths(t *testing.T) {
	...
	for _, width := range []int{minWidthRail, 60} {
		view := ansiStrip(r.renderLiveWithTools(telemetry.NewTelemetry("deepseek", "low", true, 250), defaultTheme, width, PhaseWorking, 0, log))
...
func TestRailRegressionMetersAndModelSessionTruncation(t *testing.T) {
	...
	te.Apply(telemetry.TelemetryUpdate{Kind: telemetry.TelemetryUsage, Hit: 4, Miss: 1, Output: 10, Ctx: liveContextWarnThreshold})
...
func TestRailRegressionClampBesideTranscriptAboveBottomBand(t *testing.T) {
	...
	m := NewModelCfg(Dependencies{
		Turn:      fakeSess("hi"),
		Telemetry: telemetry.NewTelemetry("deepseek", "low", true, 250),
