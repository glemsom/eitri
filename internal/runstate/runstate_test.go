...
func TestWriter_Done_BroadcastsEvent(t *testing.T) {
	...
	usage := &tokenizer.TokenUsage{TotalTokens: 100, PromptTokens: 60, CompletionTokens: 40}
	w.Done("msg-1", usage)
...
func TestWriter_ContextUpdate_BroadcastsEvent(t *testing.T) {
	...
	update := &tokenizer.ContextUpdate{
		TotalTokens:      500,
	...
	case evt := <-ch:
		...
		gotUpdate, ok := evt.Data.(*tokenizer.ContextUpdate)
		if !ok {
			t.Fatalf("event Data type = %T, want *tokenizer.ContextUpdate", evt.Data)
		}
