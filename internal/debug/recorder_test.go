...
// TestRecorder_LoadAll_AdvancesNextIDPastArchive is the acceptance test for
// issue #1236: after a restart, a recorder hydrated from the persisted archive
// must not reuse a restored trace_N ID — fresh IDs always advance past the
// archive, so a new trace can never silently overwrite a restored trace file.
func TestRecorder_LoadAll_AdvancesNextIDPastArchive(t *testing.T) {
	r := NewRecorder(10)
	r.LoadAll([]*HTTPTrace{
		{ID: "trace_3", SessionID: "s1", Method: "POST", URL: "/old", Status: 200},
		{ID: "trace_7", SessionID: "s1", Method: "POST", URL: "/old", Status: 200},
	})

	r.Record("s1", "p1", "POST", "/new", []byte(`{"model":"m1"}`), []byte(`{}`), 200, time.Second, "", nil)

	traces := r.List(0, "", "")
	if len(traces) == 0 {
		t.Fatal("no traces recorded")
	}
	newest := traces[len(traces)-1]
	if newest.ID != "trace_8" {
		t.Fatalf("new trace ID = %q, want %q (next after restored trace_7)", newest.ID, "trace_8")
	}
}

// TestRecorder_LoadAll_IgnoresNonGeneratedTraceIDs verifies that archive IDs
// that do not follow the generated trace_<n> format (e.g. hand-assigned IDs)
// never advance the generator: they cannot collide with a freshly generated
// ID, so they must not bump the counter.
func TestRecorder_LoadAll_IgnoresNonGeneratedTraceIDs(t *testing.T) {
	r := NewRecorder(10)
	r.LoadAll([]*HTTPTrace{
		{ID: "trace_a", SessionID: "s1", Status: 200},  // numeric suffix is required
		{ID: "trace_7x", SessionID: "s1", Status: 200}, // non-numeric tail
		{ID: "legacy-1", SessionID: "s1", Status: 200}, // never generated by the recorder
	})

	r.Record("s1", "p1", "POST", "/new", nil, nil, 200, 0, "", nil)

	traces := r.List(0, "", "")
	newest := traces[len(traces)-1]
	if newest.ID != "trace_1" {
		t.Fatalf("new trace ID = %q, want %q (non-generated archive IDs must not bump the counter)", newest.ID, "trace_1")
	}
}

// TestRecorder_LoadAll_DoesNotMoveNextIDBackwards verifies LoadAll only ever
// advances the generator: if the recorder has already issued IDs higher than
// the restored archive (e.g. LoadAll after live recording), it must not reset
// the counter and risk a collision.
func TestRecorder_LoadAll_DoesNotMoveNextIDBackwards(t *testing.T) {
	r := NewRecorder(10)
	r.Record("s1", "p1", "POST", "/first", nil, nil, 200, 0, "", nil)  // trace_1
	r.Record("s1", "p1", "POST", "/second", nil, nil, 200, 0, "", nil) // trace_2
	r.Record("s1", "p1", "POST", "/third", nil, nil, 200, 0, "", nil)  // trace_3

	// A stale archive (trace_1..trace_2) must not pull the counter back.
	r.LoadAll([]*HTTPTrace{{ID: "trace_1", SessionID: "s1", Status: 200}})

	r.Record("s1", "p1", "POST", "/fourth", nil, nil, 200, 0, "", nil)

	traces := r.List(0, "", "")
	newest := traces[len(traces)-1]
	if newest.ID != "trace_4" {
		t.Fatalf("new trace ID = %q, want %q (LoadAll must not move the counter backwards)", newest.ID, "trace_4")
	}
}
