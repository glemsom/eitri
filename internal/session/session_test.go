...
func TestAppendMessage_FirstUserMessageSetsTruncatedTitle(t *testing.T) {
	...
	sess, _ := mgr.Create("browser-1")
	mgr.AppendMessage(sess.ID, message.Message{Role: "user", Content: "   Fix   flaky session tab title behavior across browser tabs and runs   "})
...
func TestAppendMessage_EveryUserMessageUpdatesTitle(t *testing.T) {
	...
	sess, _ := mgr.Create("browser-1")
	mgr.AppendMessage(sess.ID, message.Message{Role: "user", Content: "first question"})
	mgr.AppendMessage(sess.ID, message.Message{Role: "assistant", Content: "answer"})
	mgr.AppendMessage(sess.ID, message.Message{Role: "user", Content: "second question should rename"})
...
func TestAppendMessage_BlankFirstUserMessageKeepsPlaceholderTitle(t *testing.T) {
	...
	sess, _ := mgr.Create("browser-1")
	mgr.AppendMessage(sess.ID, message.Message{Role: "user", Content: "   \n\t  "})
...
func TestAppendMessage_BlankFirstUserMessageDoesNotBlockLaterRename(t *testing.T) {
	...
	sess, _ := mgr.Create("browser-1")
	mgr.AppendMessage(sess.ID, message.Message{Role: "user", Content: "   \n\t  "})
	mgr.AppendMessage(sess.ID, message.Message{Role: "user", Content: "real first question"})
...
func TestAppendComponent(t *testing.T) {
	...
	comp := message.ComponentData{Name: "test_component", Data: map[string]any{"key": "val"}}
	...
	assistantMsg := message.Message{Role: "assistant", Content: "Here is the diff"}
	mgr.AppendMessage(sess.ID, assistantMsg)

	err = mgr.AppendComponent(sess.ID, message.ComponentData{Name: "test_component", Data: map[string]any{"diff": "+new code"}})
	if err != nil {
	...
	err = mgr.AppendComponent(sess.ID, message.ComponentData{Name: "mermaid", Data: map[string]any{"chart": "graph"}})
	if err != nil {
...
func TestAppendComponent_NoAssistantMessage(t *testing.T) {
	...
	mgr.AppendMessage(sess.ID, message.Message{Role: "user", Content: "hello"})

	err := mgr.AppendComponent(sess.ID, message.ComponentData{Name: "test_component", Data: nil})
	if err != nil {
...
func TestAppendComponent_NonexistentSession(t *testing.T) {
	...
	err := mgr.AppendComponent("nonexistent", message.ComponentData{Name: "test", Data: nil})
	if err != nil {
...
func TestAppendComponent_ConcurrentSafety(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func(i int) {
			defer wg.Done()
			mgr.AppendComponent(sess.ID, message.ComponentData{Name: "comp", Data: map[string]any{"i": i}})
		}(i)
	}
	wg.Wait()
...
func TestSetQuickReplies(t *testing.T) {
	...
	mgr2.AppendMessage(sess2.ID, message.Message{Role: "user", Content: "hello"})
	mgr2.AppendMessage(sess2.ID, message.Message{Role: "assistant", Content: "world"})
