...
//
// EITRI PATCH: the enable path is ignored. Upstream's scroll optimization
// (scrollOptimize in terminal_renderer_hardscroll.go) corrupts the alternate
// screen when a live region scrolls while a line's content changes at the same
// time: it leaves the old line painted in place, so Eitri's streaming
// chain-of-thought showed its header/reasoning tokens in two screen positions
// (the reported ghost). The optimization is a minor byte-count win; painting the
// changed lines individually is always correct. Remove this override when
// upstream fixes scrollOptimize (verified still broken as of
