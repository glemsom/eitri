...
# Each worker's batch session ID is auto-generated by the shared run-ID helper
# (ADR-0025); the worker's log reports its `session_id`, and the session is
...
main() {
	...
	while true; do
		...
		for num in $issues; do
			...
			# The worker's batch session ID is auto-generated by the shared run-ID
			# helper; its log reports the `session_id` and the review trail
			# (snapshot, HTTP traces, timeline) lands in `~/.eitri/sessions/<id>/`.
		...
		for entry in "${pids[@]:-}"; do
			...
			# Locate the worker's review trail: the batch session ID is
			# auto-generated, so resolve it from the run's reported `session_id`
			# rather than assuming a fixed per-issue directory.
			sid=$(grep -m1 -oE 'session_id=[A-Za-z0-9._-]+' "$WORKTREES_DIR/issue-$num/log" 2>/dev/null | head -1 | cut -d= -f2 || true)
			if [ -n "$sid" ]; then
