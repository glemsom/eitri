...
- `scripts/agent-loop.sh` workers now name their batch sessions per issue: each worker exports `EITRI_BATCH_SESSION_ID=issue-N`, so every processed issue leaves a reviewable `~/.eitri/sessions/issue-N/` (session snapshot, HTTP traces, timeline) instead of an opaque `batch-*` directory. (#1040)
