...
| **Sub-agent** | A subordinate agent loop spawned by a parent agent via the `delegate` tool; runs with its own turn loop, tool registry, and system prompt and reports back via `collect`. `delegate`/`collect` are parent-only tools, so sub-agents cannot spawn further sub-agents in v1; a browser-visible parent's delegate creates a child `UISession` (via its `ParentID` field) nested under the parent in the sidebar tree. Each sub-agent run is persisted as a child session under `~/.eitri/sessions/<taskID>/` (parent linkage, task-derived title, per-turn + terminal snapshots, traces, and timeline) in both UI and batch modes — see [ARCHITECTURE § internal/persist](docs/ARCHITECTURE.md#internalpersist--session-snapshots-traces-and-timelines-on-disk). |
...
| **Context panel** | 4th sidebar section showing live context window utilization: a progress bar in compact mode expanding to a per-category breakdown (system prompt, history, skills, completion) fed by `runstate.ComputeContext()`. The `context_update` SSE event contract and per-category token fields live in [ARCHITECTURE § internal/runstate](docs/ARCHITECTURE.md#internalrunstate--sse-broadcast--context-tracking). |
...
| **Batch run** | A headless `eitri -b` invocation that runs one prompt and exits. Persists the same trail as a UI session under `~/.eitri/sessions/<id>/` (session snapshot per turn, per-call HTTP traces, per-run timeline) so batch runs are reviewable with the same `jq`/`cat` tools and load/report paths. Session ID defaults to `batch-<unixnano>`, overridable via `EITRI_BATCH_SESSION_ID`. See ADR-0023. |
