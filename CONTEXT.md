...
| **Session persistence** | The on-disk JSON files under `~/.eitri/sessions/<id>/` and `~/.eitri/history/<id>/` that survive server restarts. Written every turn for troubleshooting/debugging. No longer restored into the in-memory session manager on startup — Eitri starts with zero active sessions. |
