...
| **Session persistence** | On-disk JSON file `~/.eitri/sessions/<id>/session.json` that survives server restarts. Written atomically every turn for troubleshooting/debugging only — not used to restore active sessions into the in-memory session manager on startup. The Persister retains up to 1 GiB of timeline and trace files, evicting the oldest across all sessions. Session messages carry all LLM-oriented fields (tool calls, tool_call_id) so the snapshot is the single source of truth. |
