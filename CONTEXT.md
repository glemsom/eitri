...
| **Session persistence** | On-disk JSON files under `~/.eitri/sessions/<id>/` and `~/.eitri/history/<id>/` that survive server restarts. Written every turn for troubleshooting/debugging only — not used to restore active sessions into the in-memory session manager on startup. The Persister retains up to 1 GiB of snapshots, evicting the oldest across all sessions. |
